...
Four channels: `dashboard.ts` (Express + WS on port 3030, REST API, file serving, optional cookie auth, password rate-limited 5/15min), `telegram.ts` (grammy long polling + photo/document attachments + `allowedChatIds`), `discord.ts` (discord.js, guild messages + DMs + slash commands), `direct.ts` (programmatic — `Orchestrator.ask()` routes through the full pipeline with `streamMode: "final"` and a 2-min timeout). Each implements `Channel` from `src/types.ts` and builds its own opaque `sessionKey`. `requireMention` is a per-channel/session toggle. Dashboard APIs are being extracted incrementally into context-driven routers under `src/routers/`; `src/routers/route.ts` centralizes Zod request validation and structured 400 responses. Drive persistence, visibility inheritance, safe site extraction, and file streaming go through the context-scoped `DriveStore` and focused drive routers. Deployed app metadata/files go through `AppStore`; PM2 status and lifecycle operations go through `AppProcessService`; app APIs and subdomain proxying live in focused app routers. Dashboard password setup/login, in-memory sessions, rate limiting, cookies, API authorization, and attachment authorization go through `DashboardAuthService` and focused auth middleware/router modules. Model discovery and provider OAuth login state, prompts, status, and logout go through `ProviderService` and focused provider routers. Dashboard filesystem reads go through `FileService`; uploaded attachment persistence and streaming go through `AttachmentStore` and focused attachment/file routers. Discord and Telegram command registration and automatic session aliases go through `ChannelManagementService` and focused channel routers. User-owned `SOUL.md` is editable through `VitoService`; project-owned `SYSTEM.md` is exposed read-only through the system-content router. The stable external `POST /api/ask` integration surface uses bearer authentication and delegates through `AskApiService` to `Orchestrator.ask()` and `DirectChannel`. Authenticated dashboard chat requests are validated by the chat router and dispatched through `DashboardChatService`; `DashboardChannel.listen()` owns handler registration. Health, runtime status, and delayed dashboard rebuild/PM2 restart operations go through `ServerLifecycleService` and its focused router.
