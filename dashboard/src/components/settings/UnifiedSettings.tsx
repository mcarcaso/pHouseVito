...
export default function UnifiedSettings() {
  ...
  const { config, loading, loadError, saveError, saving, saved, updateConfig } = useConfig();
  ...
  if (loadError || !config) {
    return <div className="p-4 text-red-400">Error: {loadError || "Failed to load config"}</div>;
  }
  ...
  return (
    <div className="flex flex-col pb-8">
      ...
      {saveError && (
        <div className="mx-4 mt-4 rounded-lg border border-red-900/60 bg-red-950/30 px-3 py-2 text-sm text-red-300">
      ...
      <div className="flex border-b border-neutral-800 px-4 sticky top-[52px] md:top-[52px] bg-[#0a0a0a] z-[9]">
        {(["global", "channels", "sessions"] as Tab[]).map((t) => (
          ...
          >
            {t === "global" ? "Global" : t === "channels" ? "Channel" : "Session"}
      ...
      <div className="p-4 max-w-2xl">
        <p className="text-xs text-neutral-600 mb-4">
          {tab === "global"
            ? "Global values are the base of the settings cascade."
            : `Override inherited settings for a specific ${tab === "channels" ? "channel" : "session"}.`}
        </p>
